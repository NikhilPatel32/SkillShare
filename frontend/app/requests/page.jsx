...
import { getMyRequests, updateRequestStatus, createConnection, isAuthenticated } from '../../utils/api';
...
export default function RequestsPage() {
  ...
  const handleCreateConnection = async () => {
    try {
      ...
      alert('Connection created successfully! You can now start skill sharing.');
    ...
    <div>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="left-2 right-5 w-auto">
          <h1 className="text-3xl font-bold mb-6 text-center">Incoming Requests</h1>
          ...
          <div className="mb-6">
            <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-screen justify-around">
              {[
                ...
                >
                  {tab.label} ({allRequests.filter(r => tab.key === 'all' || r.status === tab.key).length})
                </button>
              ))}
            </div>
          </div>
              ...
              {filteredRequests.map((request, index) => (
                <div key={index} className="bg-white border rounded-lg p-6 shadow-sm">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-semibold text-lg">{request.skillTitle}</h3>
                      <p className="text-sm text-gray-500">{request.skillCategory}</p>
                    </div>
                  ...
                  <div className="mb-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      ...
                      {request.type === 'exchange' ? ' Skill Exchange' : 'Learn Request'}
                    </span>
                  </div>
                  ...
                  {request.type === 'exchange' && request.offeredSkill && (
                    <div className="bg-blue-50 p-4 rounded-lg mb-4">
                      <h4 className="font-semibold text-blue-800 mb-2">Skill Offered in Return:</h4>
                      <div className="text-sm">
                        <p><strong>{request.offeredSkill.title}</strong></p>
                        <p className="text-gray-600">{request.offeredSkill.description}</p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConnectionData({...connectionData, skillOffered: e.target.value})}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConnectionData({...connectionData, skillReceived: e.target.value})}
                    className="w-full p-2 border rounded-md"
                    required
                  />
                </div>
